...
//    private final String STORE_PAGE = "onlineStore.jsp";
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String url = siteMap.get("storePage");
        try {
            ItemDAO dao = new ItemDAO();
            dao.loadItems();
            List<ItemDTO> result = dao.getItemList();
            request.setAttribute("ITEM_LIST", result);
        } catch (SQLException ex) {
            log("StoreViewServlet _ SQL _"+ex.getMessage());
        } catch (ClassNotFoundException ex) {
            log("StoreViewServlet _ ClassNotFound _"+ex.getMessage());
